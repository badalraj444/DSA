...
/*
ALGO:
1.init ans string , an empty stack
2. for each char in infix string:
    if operand, push to ans
    else if '(' : push
    else if ')' : pop n store untill '(' is seen , pop but dont store '('
    else(operator):
...
. visualise how there is a link between operators in stack and operands is ans as we store them. 
  ...
  to make up for this, you go aroung by popping and appending the operators instantly to the final
